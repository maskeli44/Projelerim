...
// Type: Makina_Ikmal.Sabitler.AracTipi
...
{
  public class AracTipi : RibbonForm
  {
    ...
    private GridColumn colARACTIPI;

    public AracTipi() => this.InitializeComponent();

    private void EgitimDurumu_Load(object sender, EventArgs e) => this.tAHARACTIPITableAdapter.Fill(this.bellDataSet6.TAHARACTIPI);

    private void yeniEgitimKaydetButton_ItemClick(object sender, ItemClickEventArgs e)
    {
      TAHARACTIPI entity = new TAHARACTIPI();
      entity.ARACTIPI = this.egitimBilgisi.Text;
      if (this.db.TAHARACTIPI.Where<TAHARACTIPI>((Expression<System.Func<TAHARACTIPI, bool>>) (hd => hd.ARACTIPI == this.egitimBilgisi.Text)).Count<TAHARACTIPI>() > 0)
      {
        int num = (int) MessageBox.Show("Aynı kayıt tekrar kayıt edilemez.");
      }
      else
      {
        this.db.TAHARACTIPI.Add(entity);
        this.db.SaveChanges();
      }
      this.listele();
    }
    ...
    private void listele() => this.tAHARACTIPITableAdapter.Fill(this.bellDataSet6.TAHARACTIPI);

    private void gridControl1_Click(object sender, EventArgs e)
    {
      YetkiClass.secim = "guncelle";
      YetkiClass.secim_id = Convert.ToInt32(this.gridView1.GetFocusedRowCellValue("ID"));
      this.egitimBilgisi.Text = this.gridView1.GetFocusedRowCellValue("ARACTIPI").ToString();
    }

    private void egitimSilButton_ItemClick(object sender, ItemClickEventArgs e)
    {
      this.db.TAHARACTIPI.FirstOrDefault<TAHARACTIPI>((Expression<System.Func<TAHARACTIPI, bool>>) (x => x.ID == YetkiClass.secim_id)).ARACTIPI = this.egitimBilgisi.Text;
      this.db.SaveChanges();
      this.listele();
    }
    ...
    private void InitializeComponent()
    {
      ...
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (AracTipi));
      ...
      this.colARACTIPI = new GridColumn();
      ...
      this.ribbonPage1.Text = nameof (AracTipi);
      ...
      this.gridControl1.DataSource = (object) this.tAHARACTIPIBindingSource;
      ...
      this.colARACTIPI.VisibleIndex = 0;
      this.colARACTIPI.Width = 87;
