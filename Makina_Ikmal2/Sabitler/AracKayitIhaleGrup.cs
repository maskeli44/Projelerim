...
// Type: Makina_Ikmal.Sabitler.AracKayitIhaleGrup
...
{
  public class AracKayitIhaleGrup : RibbonForm
  {
    ...
    public AracKayitIhaleGrup() => this.InitializeComponent();

    private void AracKayitIhaleGrup_Load(object sender, EventArgs e) => this.listele();

    private void yeniEgitimKaydetButton_ItemClick(object sender, ItemClickEventArgs e)
    {
      TBLARACIHALEGRUP entity = new TBLARACIHALEGRUP();
      entity.IHALEADI = this.egitimBilgisi.Text;
      if (this.db.TBLARACIHALEGRUP.Where<TBLARACIHALEGRUP>((Expression<Func<TBLARACIHALEGRUP, bool>>) (hd => hd.IHALEADI == this.egitimBilgisi.Text)).Count<TBLARACIHALEGRUP>() > 0)
      {
        int num = (int) MessageBox.Show("Aynı kayıt tekrar kayıt edilemez.");
      }
      else
      {
        this.db.TBLARACIHALEGRUP.Add(entity);
        this.db.SaveChanges();
      }
      this.listele();
    }
    ...
    {
      this.gridControl1.DataSource = (object) this.db.TBLARACIHALEGRUP.Select(hd => new
      {
        ID = hd.ID,
        IHALEADI = hd.IHALEADI
      }).ToList();
      AracEkle openForm = (AracEkle) Application.OpenForms["AracEkle"];
      if (openForm != null)
        openForm.yenile();
      else
        ((AracGuncelle) Application.OpenForms["AracGuncelle"])?.yenile();
    }
    ...
    private void gridControl1_Click(object sender, EventArgs e)
    {
      ...
      this.egitimBilgisi.Text = this.gridView1.GetFocusedRowCellValue("IHALEADI").ToString();
    }

    private void egitimSilButton_ItemClick(object sender, ItemClickEventArgs e)
    {
      this.db.TBLARACIHALEGRUP.FirstOrDefault<TBLARACIHALEGRUP>((Expression<Func<TBLARACIHALEGRUP, bool>>) (x => x.ID == YetkiClass.secim_id)).IHALEADI = this.egitimBilgisi.Text;
      this.db.SaveChanges();
      this.listele();
    }
    ...
    private void InitializeComponent()
    {
      ...
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (AracKayitIhaleGrup));
      ...
      this.ribbonPage1.Text = "Araç İhale Grubu";
      ...
      this.colGRUPADI.Caption = "İhale Adı";
      this.colGRUPADI.FieldName = "IHALEADI";
      ...
      this.labelControl1.Size = new Size(101, 18);
      this.labelControl1.TabIndex = 6;
      this.labelControl1.Text = "Araç İhale Grup";
      ...
      this.Name = nameof (AracKayitIhaleGrup);
      this.Ribbon = this.ribbon;
      this.StatusBar = this.ribbonStatusBar;
      this.Text = "Araç  İhale Grup Girişi";
      this.Load += new EventHandler(this.AracKayitIhaleGrup_Load);
      this.ribbon.EndInit();
