...
// Type: Makina_Ikmal.Sabitler.BirimAdi
...
{
  public class BirimAdi : RibbonForm
  {
    ...
    public BirimAdi() => this.InitializeComponent();

    private void EgitimDurumu_Load(object sender, EventArgs e) => this.tBLBELDBIRIMLERTableAdapter.Fill(this.bellDataSet5.TBLBELDBIRIMLER);

    private void yeniEgitimKaydetButton_ItemClick(object sender, ItemClickEventArgs e)
    {
      TBLBELDBIRIMLER entity = new TBLBELDBIRIMLER();
      entity.BELDBIRIMADI = this.egitimBilgisi.Text;
      if (this.db.TBLBELDBIRIMLER.Where<TBLBELDBIRIMLER>((Expression<System.Func<TBLBELDBIRIMLER, bool>>) (hd => hd.BELDBIRIMADI == this.egitimBilgisi.Text)).Count<TBLBELDBIRIMLER>() > 0)
      {
        int num = (int) MessageBox.Show("Aynı kayıt tekrar kayıt edilemez.");
      }
      else
      {
        this.db.TBLBELDBIRIMLER.Add(entity);
        this.db.SaveChanges();
      }
      this.listele();
    }
    ...
    private void listele() => this.tBLBELDBIRIMLERTableAdapter.Fill(this.bellDataSet5.TBLBELDBIRIMLER);

    private void gridControl1_Click(object sender, EventArgs e)
    {
      YetkiClass.secim = "guncelle";
      YetkiClass.secim_id = Convert.ToInt32(this.gridView1.GetFocusedRowCellValue("ID"));
      this.egitimBilgisi.Text = this.gridView1.GetFocusedRowCellValue("BELDBIRIMADI").ToString();
    }

    private void egitimSilButton_ItemClick(object sender, ItemClickEventArgs e)
    {
      this.db.TBLBELDBIRIMLER.FirstOrDefault<TBLBELDBIRIMLER>((Expression<System.Func<TBLBELDBIRIMLER, bool>>) (x => x.ID == YetkiClass.secim_id)).BELDBIRIMADI = this.egitimBilgisi.Text;
      this.db.SaveChanges();
      this.listele();
    }
    ...
    private void InitializeComponent()
    {
      ...
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (BirimAdi));
      ...
      this.gridControl1.DataSource = (object) this.tBLBELDBIRIMLERBindingSource;
      ...
      this.colBELDBIRIMADI.MinWidth = 23;
      this.colBELDBIRIMADI.Name = "colBELDBIRIMADI";
      this.colBELDBIRIMADI.OptionsColumn.AllowEdit = false;
      this.colBELDBIRIMADI.OptionsColumn.ReadOnly = true;
      this.colBELDBIRIMADI.Visible = true;
      this.colBELDBIRIMADI.VisibleIndex = 0;
      this.colBELDBIRIMADI.Width = 87;
