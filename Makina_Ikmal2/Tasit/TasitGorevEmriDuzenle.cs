...
// Type: Makina_Ikmal.Tasit.TasitGorevEmriDuzenle
...
namespace Makina_Ikmal.Tasit
{
  public class TasitGorevEmriDuzenle : XtraForm
  {
    ...
    private void Duzenle()
    {
      try
      {
        if (this.BoslukControl())
        {
          ...
          this.mesaiSuresi = Convert.ToInt32((this.donusTarihDateEdit.DateTime - this.cikisTarihDateEdit.DateTime).TotalHours) - 8;
          if (this.mesaiSuresi <= 0)
            this.mesaiSuresi = 0;
          ...
          if (this.cikisTarihDateEdit.Text != "")
            tbltasitgorev.CIKISZAMANI = new DateTime?(Convert.ToDateTime(this.cikisTarihDateEdit.EditValue));
          if (this.cikisKmTxt.Text != "")
            tbltasitgorev.CIKISKM = this.cikisKmTxt.Text;
          ...
          tbltasitgorev.MESAIMIKTARI = this.mesaiSuresi.ToString();
          this.db.SaveChanges();
        ...
        {
          int num1 = (int) XtraMessageBox.Show("Eksik girdi veya hatalı giriş yaptınız.");
        }
      }
      catch (Exception ex)
      {
    ...
    {
      tasitPersonelEkle tasitPersonelEkle = new tasitPersonelEkle();
      tasitPersonelEkle.Show();
      tasitPersonelEkle.hangisi = 2;
    }
      ...
      try
      {
        if (this.BoslukControl())
        {
          ...
          foreach (var data in source2.Select(selector2))
            str2 = data.UNVAN;
          Parameter parameter1 = this.yeniMesaiRapor.Parameters["iseBaslama"];
          DateTime dateTime = this.cikisTarihDateEdit.DateTime;
          string str3 = dateTime.ToString();
          parameter1.Value = (object) str3;
          this.yeniMesaiRapor.Parameters["iseBaslama"].Visible = false;
          Parameter parameter2 = this.yeniMesaiRapor.Parameters["isiBitirme"];
          dateTime = this.donusTarihDateEdit.DateTime;
          ...
          Parameter parameter3 = this.yeniMesaiRapor.Parameters["simdikiTarih"];
          dateTime = DateTime.Now;
          string str5 = dateTime.ToShortDateString().ToString();
          parameter3.Value = (object) str5;
    ...
    public void guncelleLoad(int guncelleID)
    {
      ...
      Expression<System.Func<TBLTASITGOREV, \u003C\u003Ef__AnonymousType77<string, int?, int?, string, string, string, DateTime?, int?, DateTime?, string, DateTime?, string, int?, string, int?, string, string, int?>>> selector = hd => new
      {
        KOD = hd.KOD,
        PLAKAID = hd.PLAKAID,
        SOFORID = hd.SOFORID,
        ...
        MUDONAYZAMAN = hd.MUDONAYZAMAN,
        GOREVGRUPID = hd.GOREVGRUPID
      };
      foreach (var data in source.Select(selector))
      {
        ...
        this.gorevVerenCombo.SelectedIndex = Convert.ToInt32((object) data.GOREVGRUPID);
    ...
    private void InitializeComponent()
    {
      ...
      this.radioGroup1.Location = new Point(507, -4);
      ...
      this.donusTarihDateEdit.Properties.Mask.UseMaskAsDisplayFormat = true;
