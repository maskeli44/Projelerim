...
// Type: Makina_Ikmal.YagBolum.YagEmri
  ...
  public class YagEmri : XtraForm
  {
    ...
    private void guncelle()
    {
      try
      {
        ...
        if (this.bakimozeti.Text != "")
          ise.YAPILANBAKIMOZETI = this.bakimozeti.Text;
        if (this.aracigetiren.Text != "")
        ...
        if (this.HidrolikYagTxt.Text != "")
          tblaraclar.HIDROLIKYAGKM = this.HidrolikYagTxt.Text;
        this.db.TBLISEMRIBAKIMLAR.Where<TBLISEMRIBAKIMLAR>((Expression<System.Func<TBLISEMRIBAKIMLAR, bool>>) (hd => hd.ISEMRIID == (int?) ise.ID));
        for (int rowHandle = 0; rowHandle < this.gridView6.DataRowCount; ++rowHandle)
        {
      ...
      try
      {
        if (source.Any<TBLISEMRIBAKIMLAR>())
        {
          foreach (TBLISEMRIBAKIMLAR tblisemribakimlar in (IEnumerable<TBLISEMRIBAKIMLAR>) source)
          {
            ...
            this.db.TBLISEMRIBAKIMLAR.Remove(this.db.TBLISEMRIBAKIMLAR.FirstOrDefault<TBLISEMRIBAKIMLAR>((Expression<System.Func<TBLISEMRIBAKIMLAR, bool>>) (x => x.ID == item.ID)));
          }
        }
